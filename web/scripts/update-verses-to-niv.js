 ...
 * Uses the passages endpoint so multi-verse ranges work and verse numbers
 * are not embedded in the returned text.
 */
async function fetchNIVVerse(reference) {
  try {
    ...
    // Parse reference — handles "John 3:16" and "John 3:16-18"
    ...
    const bookCode = BOOK_MAP[book.trim().toLowerCase()] || book.trim().toUpperCase().substring(0, 3);
    
    // Passages endpoint requires the full range form: BOOK.ch.v1-BOOK.ch.v2
    // (the verses endpoint does not support ranges at all)
    const passageId = endVerse
    ...
    // With content-type=text the content is already plain text; just normalise whitespace
...
async function updateVersesToNIV() {
  ...
  for (const verse of verses) {
    ...
    // Delay to avoid rate limiting (2.5 seconds between requests)
